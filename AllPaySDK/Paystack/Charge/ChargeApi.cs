...
namespace AllPaySDK.Paystack.Charge
{
    public class ChargeApi : IChargeApi
    {
        ...
        public ChargeResponse ChargeAuthorizationCode(AuthorizationCodeChargeRequest request, bool makeReferenceUnique = false)
        {
            ...
            return _api.Post<ChargeResponse>("charge", request);
        }
        ...
        public ChargeResponse SubmitBirthday(string reference, DateTime birthday) => _api.Post<ChargeResponse>("charge/submit_birthday", new
        {
            birthday,
            reference
        });

        public ChargeResponse SubmitOTP(string reference, string otp) => _api.Post<ChargeResponse>("charge/submit_otp", new
        {
            otp,
            reference
        });

        public ChargeResponse SubmitPhone(string reference, string phone) => _api.Post<ChargeResponse>("charge/submit_phone", new
        {
            phone,
            reference
        });

        public ChargeResponse SubmitPIN(string reference, string pin) => _api.Post<ChargeResponse>("charge/submit_pin", new
        {
            pin,
            reference
        });
        ...
        public ChargeTokenizeResponse Tokenize(string email, string cardNumber, string cardCvv, string cardExpiryMonth, string cardExpiryYear) =>
            _api.Post<ChargeTokenizeResponse>("charge/tokenize", new
            {
                email,
                card = new
                {
